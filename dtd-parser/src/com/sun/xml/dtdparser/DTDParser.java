...
//    private SimpleHashtable    elements = new SimpleHashtable (47);
	...
	public void setLocale( Locale l ) throws SAXException
	{
		...
		if( l != null && !messages.isLocaleSupported(l.toString()) )
		{
			throw new SAXException(
					messages.getMessage(locale, "P-078", new Object[]
					{
							l
					})
			);
	 ...
	 *                  "fr-ca", followed by "zh_CN". Both RFC 1766 and Java
			...
			throws IOException, SAXException
	{
		...
		catch( EndOfInputException e )
		{
			if( !in.isDocument() )
			{
				...
				do
				{ // force a relevant URI and line number
					in = in.pop();
				}
				while( in.isInternal() );
		...
		{
			LOGGER.error("Internal DTD parser error.", e);

			throw new SAXParseException(
					e.getMessage() != null ? e.getMessage()
							: e.getClass().getName(),
					getPublicId(), getSystemId(), getLineNumber(),
					getColumnNumber()
			);
...
//    isInAttribute = false;
	...
	) throws IOException, SAXException
	{
		...
		else if( entity == null )
		{
			...
			//
			fatal((table == params) ? "V-022" : "P-014", new Object[]
			{
					name
			});
		}
	...
	private boolean maybeMarkupDecl() throws IOException, SAXException
	{
		...
		return maybeElementDecl() || maybeAttlistDecl() || maybeEntityDecl()
				|| maybeNotationDecl() || maybePI(false) || maybeComment(false);
	}
	 ...
	 * <p>
	 * <P>
	 * Separately, make sure IDREF values match some ID provided in the document
	 * (in the afterRoot method).
	 */
	/*
	 * void validateAttributeSyntax (Attribute attr, String value) throws
	 * DTDParseException { // ID, IDREF(S) ... values are Names if (Attribute.ID
	 * == attr.type()) { if (!XmlNames.isName (value)) error ("V-025", new
	 * Object [] { value });
	 * 
	 * Boolean b = (Boolean) ids.getNonInterned (value); if (b == null ||
	 * b.equals (Boolean.FALSE)) ids.put (value.intern (), Boolean.TRUE); else
	 * error ("V-026", new Object [] { value });
	 * 
	 * } else if (Attribute.IDREF == attr.type()) { if (!XmlNames.isName
	 * (value)) error ("V-027", new Object [] { value });
	 * 
	 * Boolean b = (Boolean) ids.getNonInterned (value); if (b == null) ids.put
	 * (value.intern (), Boolean.FALSE);
	 * 
	 * } else if (Attribute.IDREFS == attr.type()) { StringTokenizer tokenizer =
	 * new StringTokenizer (value); Boolean b; boolean sawValue = false;
	 * 
	 * while (tokenizer.hasMoreTokens ()) { value = tokenizer.nextToken (); if
	 * (!XmlNames.isName (value)) error ("V-027", new Object [] { value }); b =
	 * (Boolean) ids.getNonInterned (value); if (b == null) ids.put
	 * (value.intern (), Boolean.FALSE); sawValue = true; } if (!sawValue) error
	 * ("V-039", null);
	 * 
	 * 
	 * // NMTOKEN(S) ... values are Nmtoken(s) } else if (Attribute.NMTOKEN ==
	 * attr.type()) { if (!XmlNames.isNmtoken (value)) error ("V-028", new
	 * Object [] { value });
	 * 
	 * } else if (Attribute.NMTOKENS == attr.type()) { StringTokenizer tokenizer
	 * = new StringTokenizer (value); boolean sawValue = false;
	 * 
	 * while (tokenizer.hasMoreTokens ()) { value = tokenizer.nextToken (); if
	 * (!XmlNames.isNmtoken (value)) error ("V-028", new Object [] { value });
	 * sawValue = true; } if (!sawValue) error ("V-032", null);
	 * 
	 * // ENUMERATION ... values match one of the tokens } else if
	 * (Attribute.ENUMERATION == attr.type()) { for (int i = 0; i <
	 * attr.values().length; i++) if (value.equals (attr.values()[i])) return;
	 * error ("V-029", new Object [] { value });
	 * 
	 * // NOTATION values match a notation name } else if (Attribute.NOTATION ==
	 * attr.type()) { // // XXX XML 1.0 spec should probably list references to
	 * // externally defined notations in standalone docs as // validity errors.
	 * Ditto externally defined unparsed // entities; neither should show up in
	 * attributes, else // one needs to read the external declarations in order
	 * // to make sense of the document (exactly what tagging // a doc as
	 * "standalone" intends you won't need to do). // for (int i = 0; i <
	 * attr.values().length; i++) if (value.equals (attr.values()[i])) return;
	 * error ("V-030", new Object [] { value });
	 * 
	 * // ENTITY(IES) values match an unparsed entity(ies) } else if
	 * (Attribute.ENTITY == attr.type()) { // see note above re standalone if
	 * (!isUnparsedEntity (value)) error ("V-031", new Object [] { value });
	 * 
	 * } else if (Attribute.ENTITIES == attr.type()) { StringTokenizer tokenizer
	 * = new StringTokenizer (value); boolean sawValue = false;
	 * 
	 * while (tokenizer.hasMoreTokens ()) { value = tokenizer.nextToken (); //
	 * see note above re standalone if (!isUnparsedEntity (value)) error
	 * ("V-031", new Object [] { value }); sawValue = true; } if (!sawValue)
	 * error ("V-040", null);
	 * 
	 * } else if (Attribute.CDATA != attr.type()) throw new InternalError
	 * (attr.type()); }
	 */
	/*
	 * private boolean isUnparsedEntity (String name) { Object e =
	 * entities.getNonInterned (name); if (e == null || !(e instanceof
	 * ExternalEntity)) return false; return ((ExternalEntity)e).notation !=
	 * null; }
	 */
...
//    boolean        declEffective = false;
		...
		/*
		 * if (element != null) { if (element.contentModel() != null) { error
		 * ("V-012", new Object [] { name }); } // else <!ATTLIST name ...> came
		 * first } else { element = new Element(name); elements.put
		 * (element.name(), element); declEffective = true; }
		 */
		if( declaredElements.contains(name) )
		{
			error("V-012", new Object[]
			{
					name
			});
		}
...
///        // leave element.contentModel as null for this case.
			dtdHandler.startContentModel(
					name, modelType = DTDEventListener.CONTENT_MODEL_EMPTY
			);
		}
		else if( peek(strANY) )
		{
///        element.setContentModel(new StringModel(StringModelType.ANY));
			dtdHandler.startContentModel(
					name, modelType = DTDEventListener.CONTENT_MODEL_ANY
			);
		}
		else
		{
...
///        dtdHandler.elementDecl(element);
			...
			throws IOException, SAXException
	{
		...
		if( peek("#PCDATA") )
		{
			...
			dtdHandler.startContentModel(
					elementName,
					modelType = DTDEventListener.CONTENT_MODEL_MIXED
			);
		...
		else
		{
			dtdHandler.startContentModel(
					elementName,
					modelType = DTDEventListener.CONTENT_MODEL_CHILDREN
			);
	...
	private void getcps( /* Element element, */String elementName,
			InputEntity start
	) throws IOException, SAXException
	{
...
//                }
				...
				{
					fatal((type == 0) ? "P-041" : "P-040", new Object[]
					{
							Character.valueOf(c), Character.valueOf(type)
					});
				}
			}
			else
...
//        return retval;
	...
	private void getMixed(
			String elementName, /* Element element, */ InputEntity start
	) throws IOException, SAXException
	{
...
//    l.add(new StringModel(StringModelType.PCDATA));
		...
		if( !peek("\u0029*") ) // right paren
		{
			...
			{
					elementName, Character.valueOf(getc())
			});
		}
		if( in != start )
		{
...
//    }
		...
		while( !peek(">") )
		{
			...
			if( c == '%' )
			{
				...
				LOGGER.debug("Parsed attribute entity name: %s", entityName);
			...
			// LOGGER.debug("Parsed attribute name: %s", attName);

			if( attName == null )
			{
				fatal("P-044", new Object[]
				{
						attName + " " + in.getLineNumber() + " "
								+ Character.valueOf(getc()).toString()
				});
			}
...
///                a.setValue(i, (String)v.elementAt(i));
			...
			else
			{
				...
				{
						attName, Character.valueOf(getc())
				});
				typeName = null;
			}
...
///        }
			...
			String[] v = (values != null)
					? (String[]) values.toArray(new String[ values.size() ])
					: null;
			dtdHandler.attributeDecl(
					elementName, attName, typeName, v, attributeUse,
					defaultValue
			);
			maybeWhitespace();
		}
	...
	private boolean maybeEntityDecl() throws IOException, SAXException
	{
		...
		if( externalId == null )
		{
			...
			if( doStore )
			{
				...
				if( defns == entities )
				{
					dtdHandler.internalGeneralEntityDecl(
							entityName, new String(value)
					);
				}
			}
			...
			if( doStore )
			{
				...
				if( externalId.notation != null )
				{
					dtdHandler.unparsedEntityDecl(
							entityName, externalId.publicId,
							externalId.systemId, externalId.notation
					);
				}
				else if( defns == entities )
				{
					dtdHandler.externalGeneralEntityDecl(
							entityName, externalId.publicId, externalId.systemId
					);
				}
			}
		}
	...
	private char getc() throws IOException, SAXException
	{
		...
		if( (c = in.getc()) == '%' && doLexicalPE )
		{
			...
			else if( entity instanceof ExternalEntity ) // PEs can't be
														// unparsed!
														// XXX if this returns
														// false ...
			{
			...
			else if( entity == null ) // see note in maybePEReference re making
										// this be
										// nonfatal.
			{
				fatal("V-022");
			...
			throws IOException, SAXException
	{
		...
		if( !in.peekc(c) )
		{
			...
			{
					Character.valueOf(c), messages.getMessage(locale, location),
					(near == null ? "" : ('"' + near + '"'))
			});
		}
	}
			...
			throws IOException, SAXException
	{
		...
		{
			String msg = "unable to open the external entity from :"
					+ next.systemId;
			...
			SAXParseException spe = new SAXParseException(
					msg, getPublicId(), getSystemId(), getLineNumber(),
					getColumnNumber(), e
			);
			...
			throws SAXException
	{
		...
		SAXParseException e = new SAXParseException(
				messages.getMessage(locale, messageId, parameters), getPublicId(), getSystemId(), getLineNumber(), getColumnNumber()
		);

		LOGGER.warn(messages.getMessage(locale, messageId, parameters), e);
	...
	void error( String messageId, Object parameters[] ) throws SAXException
	{
		...
		LOGGER.error(messages.getMessage(locale, messageId, parameters), e);
		dtdHandler.error(e);
	}
