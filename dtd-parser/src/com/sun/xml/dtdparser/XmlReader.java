...
/**
 * This handles several XML-related tasks that normal java.io Readers don't
 * support, inluding use of IETF standard encoding names and automatic detection
 * of most XML encodings. The former is needed for interoperability; the latter
 * is needed to conform with the XML spec. This class also optimizes reading
 * some common encodings by providing low-overhead unsynchronized Reader
 * support.
 * <p/>
 * <P>
 * Note that the autodetection facility should be used only on data streams
 * which have an unknown character encoding. For example, it should never be
 * used on MIME text/xml entities.
 * <p/>
 * <P>
 * Note that XML processors are only required to support UTF-8 and UTF-16
 * character encodings. Autodetection permits the underlying Java implementation
 * to provide support for many other encodings, such as US-ASCII, ISO-8859-5,
...
final class XmlReader extends Reader
{
	...
	/**
	 * Constructs the reader from an input stream, auto-detecting the encoding
	 * to use according to the heuristic specified in the XML 1.0
	 * recommendation.
	 *
	...
	/**
	 * Creates a reader supporting the given encoding, mapping from standard
	 * encoding names to ones that understood by Java where necessary.
	 *
	 * @param in       the input stream from which the reader is constructed
	 * @param encoding the IETF standard name of the encoding to use; if null,
	...
	private XmlReader( InputStream stream ) throws IOException
	{
		...
		if( len == 4 )
			...
			case 0x4c:
				if( buf[1] == 0x6f && (0x0ff & buf[2]) == 0x0a7
						&& (0x0ff & buf[3]) == 0x094 )
				{
	...
	/*
	 * Read the encoding decl on the stream, knowing that it should be readable
	 * using the specified encoding (basically, ASCII or EBCDIC). The body of
	 * the document may use a wider range of characters than the XML/Text decl
	 * itself, so we switch to use the specified encoding as soon as we can.
	 * (ASCII is a subset of UTF-8, ISO-8859-*, ISO-2022-JP, EUC-JP, and more;
	 * EBCDIC has a variety of "code pages" that have these characters as a
	 * common subset.)
	 */
			...
			throws IOException
	{
		...
		r = new InputStreamReader(
				new ByteArrayInputStream(buffer, 4, len), encoding
		);
	...
	/**
	 * Sets a mark allowing a limited number of characters to be "peeked", by
	 * reading and then resetting.
	 *
	...
	static abstract class BaseReader extends Reader
	{
		...
		public boolean ready() throws IOException
		{
			return instream == null || (finish - start) > 0
					...
					else if( (buffer[off] & 0x0f8) == 0x0F0 )
					{
						...
						if( c > 0x0010ffff )
							throw new CharConversionException(
									"UTF-8 encoding of character 0x00" + Integer
											.toHexString(c)
