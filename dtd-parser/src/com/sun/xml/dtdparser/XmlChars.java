...
public class XmlChars
{
	...
	/**
	 * Returns true if the argument, a UCS-4 character code, is valid in XML
	 * documents. Unicode characters fit into the low sixteen bits of a UCS-4
	 * character, and pairs of Unicode <em>surrogate characters</em> can be
	 * combined to encode UCS-4 characters in documents containing only Unicode.
	 * (The <code>char</code> datatype in the Java Programming Language
	 * represents Unicode characters, including unpaired surrogates.)
	 * <p>
	 * <P>
	 * In XML, UCS-4 characters can also be encoded by the use of <em>character
	 * references</em> such as <b>&amp;#x12345678;</b>, which happens to refer
	 * to a character that is disallowed in XML documents. UCS-4 characters
	 * allowed in XML documents can be expressed with one or two Unicode
	 * characters.
	 *
	 ...
	 * // Zs LINE_SEPARATOR = 13, // Zl PARAGRAPH_SEPARATOR = 14, // Zp CONTROL
	 * = 15, // Cc FORMAT = 16, // Cf // 17 reserved for proposed Ci category
	 * PRIVATE_USE = 18, // Co SURROGATE = 19, // Cs DASH_PUNCTUATION = 20, //
	 * Pd START_PUNCTUATION = 21, // Ps END_PUNCTUATION = 22, // Pe
	 * CONNECTOR_PUNCTUATION = 23, // Pc OTHER_PUNCTUATION = 24, // Po
	 * MATH_SYMBOL = 25, // Sm CURRENCY_SYMBOL = 26, // Sc MODIFIER_SYMBOL = 27,
	 * // Sk OTHER_SYMBOL = 28; // So
	 */

	/**
	 * Returns true if the character is an XML "letter". XML Names must start
	 * with Letters or a few other characters, but other characters in names
	 * must only satisfy the <em>isNameChar</em> predicate.
	 *
	 * @see #isNameChar(char)
	 * @see #isNCNameChar(char)
	 */
	public static boolean isLetter( char c )
	{
		...
		default:
			...
			return((c >= 0x02bb && c <= 0x02c1) || c == 0x0559 || c == 0x06e5
					|| c == 0x06e6);
		}
	}
	...
	private static boolean isCompatibilityChar( char c )
	{
		...
		case 0x01:
			...
			return (c >= 0x0132 && c <= 0x0133) || (c >= 0x013f && c <= 0x0140)
					|| c == 0x0149 || c == 0x017f
					|| (c >= 0x01c4 && c <= 0x01cc)
					|| (c >= 0x01f1 && c <= 0x01f3);
		...
		case 0x11:
			...
			return c == 0x1101 || c == 0x1104 || c == 0x1108 || c == 0x110a
					|| c == 0x110d || (c >= 0x1113 && c <= 0x113b)
					|| c == 0x113d || c == 0x113f
					|| (c >= 0x1141 && c <= 0x114b) || c == 0x114d
					|| c == 0x114f || (c >= 0x1151 && c <= 0x1153)
					|| (c >= 0x1156 && c <= 0x1158) || c == 0x1162
					|| c == 0x1164 || c == 0x1166 || c == 0x1168
					|| (c >= 0x116a && c <= 0x116c)
					|| (c >= 0x116f && c <= 0x1171) || c == 0x1174
					|| (c >= 0x1176 && c <= 0x119d)
					|| (c >= 0x119f && c <= 0x11a2)
					|| (c >= 0x11a9 && c <= 0x11aa)
					|| (c >= 0x11ac && c <= 0x11ad)
					|| (c >= 0x11b0 && c <= 0x11b6) || c == 0x11b9
					|| c == 0x11bb || (c >= 0x11c3 && c <= 0x11ea)
					|| (c >= 0x11ec && c <= 0x11ef)
					|| (c >= 0x11f1 && c <= 0x11f8);
