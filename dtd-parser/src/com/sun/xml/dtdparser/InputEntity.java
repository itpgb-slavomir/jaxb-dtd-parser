...
/**
 * This is how the parser talks to its input entities, of all kinds. The
 * entities are in a stack.
 * <p>
 * <P>
 * For internal entities, the character arrays are referenced here, and read
 * from as needed (they're read-only). External entities have mutable buffers,
 * that are read into as needed.
 * <p>
 * <P>
 * <em>Note:</em> This maps CRLF (and CR) to LF without regard for whether it's
 * in an external (parsed) entity or not. The XML 1.0 spec is inconsistent in
 * explaining EOL handling; this is the sensible way.
 *
...
public class InputEntity
{
	...
	//
	public void init(
			InputSource in, String name, InputEntity stack, boolean isPE
	) throws IOException, SAXException
	{
		...
		if( reader == null )
		{
			...
			if( bytes == null )
				if( Boolean.valueOf(
						System.getProperty("enableExternalEntityProcessing")
				) )
					reader = XmlReader.createReader(
							new URL(in.getSystemId()).openStream()
					);
				else
					fatal("P-082", new Object[]
					{
							in.getSystemId()
					});
			else if( in.getEncoding() != null )
				reader = XmlReader
						.createReader(in.getByteStream(), in.getEncoding());
			else
	...
	/**
	 * Returns the name of the encoding in use, else null; the name returned is
	 * in as standard a form as we can get.
	 */
	...
	/**
	 * returns the next name char, or NUL ... faster than getc(), and the common
	 * "name or nmtoken must be next" case won't need ungetc().
	 */
	...
	/**
	 * gets the next Java character -- might be part of an XML text character
	 * represented by a surrogate pair, or be the end of the entity.
	 */
	...
	/**
	 * normal content; whitespace in markup may be handled specially if the
	 * parser uses the content model.
	 * <p>
	 * <P>
	 * content terminates with markup delimiter characters, namely ampersand
	 * (&amp;amp;) and left angle bracket (&amp;lt;).
	 * <p>
	 * <P>
	 * the document handler's characters() method is called on all the content
	 * found
	 */
	public boolean parsedContent( DTDEventListener docHandler
	/* ElementValidator validator */ ) throws IOException, SAXException
	{
...
//    validator.text ();
	 ...
	 * including unescaped markup delimiters (ampersand and left angle bracket).
	 * This should otherwise be exactly like character data, modulo differences
	 * in error report details.
	 * <p>
	 * <P>
	 * The document handler's characters() or ignorableWhitespace() methods are
	 * invoked on all the character data found
	 *
	 * @param docHandler               gets callbacks for character data
	 * @param ignorableWhitespace      if true, whitespace characters will be
	 *                                 reported using
	 *                                 docHandler.ignorableWhitespace();
	 *                                 implicitly, non-whitespace characters
	 *                                 will cause validation errors
	 * @param whitespaceInvalidMessage if true, ignorable whitespace causes a
	 *                                 validity error report as well as a
	 *                                 callback
	 */
	public boolean unparsedContent(
			DTDEventListener docHandler,
			/* ElementValidator validator, */
			boolean ignorableWhitespace, String whitespaceInvalidMessage
	) throws IOException, SAXException
	{
			...
			for( last = start; last < finish; last++ )
			{
				...
				if( c == '\r' )
				{
					...
					if( white )
					{
						...
						docHandler
								.ignorableWhitespace(buf, start, last - start);
			...
			if( white )
			{
				if( whitespaceInvalidMessage != null && errHandler != null )
					errHandler.error(
							new SAXParseException(
									DTDParser.messages.getMessage(
											locale, whitespaceInvalidMessage
									), null
							)
					);
...
//        validator.text ();
	 ...
	 * <p>
	 * <P>
	 * the document handler's ignorableWhitespace() method is called on all the
	 * whitespace found
	 */
...
// XXX handles Macintosh line endings wrong
	...
	/**
	 * returns false iff 'next' string isn't as provided, else skips that text
	 * and returns true.
	 * <p>
	 * <P>
	 * NOTE: two alternative string representations are both passed in, since
	 * one is faster.
	 */
	...
	private void fatal( String messageId, Object params[] ) throws SAXException
	{
		...
		SAXParseException x = new SAXParseException(
				DTDParser.messages.getMessage(locale, messageId, params), null
		);
